...
public abstract class TemplateTSP implements TSP {
	...
	public void chercheSolution(int tpsLimite, int nbSommets, int[][] cout, int[] duree) {
		...
		calculateNearestPointPath(nonVus, cout, duree);
		...
		if (nonVus.size() == 0) { // tous les sommets ont ete visites
			...
			while (it.hasNext()) {
				...
				branchAndBound(prochainSommet, nonVus, vus, coutVus
						...
						+ duree[prochainSommet], cout, duree, tpsDebut,
						tpsLimite);
	...
	void calculateNearestPointPath(ArrayList<Integer> nonVus, int[][] cout, int[] duree) {
		int value=0;
		
		ArrayList<Integer> nonVusTemp = (ArrayList<Integer>) nonVus.clone();
		int sommetCourantTemp=0;
		meilleureSolution[0]=0;
		int c=1;
		while(nonVusTemp.size()!=0){
			int min=cout[sommetCourantTemp][nonVusTemp.get(0)];
			int next=0;
			
			for(int i=1;i<nonVusTemp.size();++i){
				if(cout[sommetCourantTemp][nonVusTemp.get(i)]<min){
					min=cout[sommetCourantTemp][nonVusTemp.get(i)];
					next=i;
				}
			}
			value+=cout[sommetCourantTemp][nonVusTemp.get(next)]+duree[nonVusTemp.get(next)];
			sommetCourantTemp=nonVusTemp.get(next);
			meilleureSolution[c]=nonVusTemp.get(next);
			nonVusTemp.remove(next);
			c++;
			
		}
		value+=cout[sommetCourantTemp][0];
		coutMeilleureSolution = value;
	}
}
